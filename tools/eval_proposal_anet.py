...
class ANETproposal(object):
    ...
    def evaluate(self):
        ...
        precision, avg_precision, proposals_per_video = average_precision_vs_avg_nr_proposals(
            self.ground_truth, self.proposal,
            max_avg_nr_proposals=self.max_avg_nr_proposals,
            tiou_thresholds=self.tiou_thresholds)

        f_value = 2 * precision * recall / (precision + recall)
        avg_f_value = f_value.mean(axis=0)
...
def average_precision_vs_avg_nr_proposals(ground_truth, proposals, max_avg_nr_proposals=None,
                                          tiou_thresholds=np.linspace(0.5, 0.95, 10)):
    """ Computes the average precision given an average number
        of proposals per video.
    ...
    -------
    precision : 2darray
        precision[i,j] is precision at ith tiou threshold at the jth average number of average number of proposals per video.
    average_precision : 1darray
        precision averaged over a list of tiou threshold. This is equivalent to precision.mean(axis=0).
    proposals_per_video : 1darray
        average number of proposals per video.
    """
    ...
    max_avg_nr_proposals * float(video_lst.shape[0]) / total_nr_proposals)
    matches = np.empty((video_lst.shape[0], pcn_lst.shape[0]))
    positives = np.arange(1, max_avg_nr_proposals + 1)
    precision = np.empty((tiou_thresholds.shape[0], pcn_lst.shape[0]))
    # Iterates over each tiou threshold.
    for ridx, tiou in enumerate(tiou_thresholds):
        ...
        # Computes precision given the set of matches per video.
        precision[ridx, :] = matches.mean(axis=0) / positives
    ...
    return precision, avg_precision, proposals_per_video
